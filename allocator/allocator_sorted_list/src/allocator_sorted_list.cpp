...
[[nodiscard]] void *allocator_sorted_list::allocate(size_t value_size,size_t values_count)
{
    ...
    *size_space = size_before - requested_size - sizeof(size_t) - sizeof(void*);
    ...
    information_with_guard(get_typename() + " available sum size" + std::to_string(*size_space));
    ...
    debug_with_guard(get_typename() + "method allocate has finished");
...
void allocator_sorted_list::deallocate(void *at)
{
    ...
    trace_with_guard(get_typename() + "method deallocate has started");
    ...
    {
        std::string is_oc = value.is_block_occupied ? "not free" : "free";
